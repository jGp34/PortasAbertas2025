// --- STEP (oTropiAttack) ---
// Self-rescue
if (place_meeting(x, y, oObstacle)) {
    var _pushed_out = false;
    for (var i = 0; i < 32; i++) {
        if (!place_meeting(x - i, y, oObstacle)) { x -= i; _pushed_out = true; break; }
    }
    if (!_pushed_out) {
        for (var i = 0; i < 32; i++) {
            if (!place_meeting(x + i, y, oObstacle)) { x += i; break; }
        }
    }
}

// Gravity
speed_v += gravity_effect;
speed_v = clamp(speed_v, -max_speed_v, max_speed_v);

// Remainder handling
x_remainder += speed_h;
y_remainder += speed_v;
var _move_x = round(x_remainder);
var _move_y = round(y_remainder);
x_remainder -= _move_x;
y_remainder -= _move_y;

// Horizontal move
if (_move_x != 0) {
    var _dir = sign(_move_x);
    repeat(abs(_move_x)) {
        if (place_meeting(x + _dir, y, oObstacle)) {
            speed_h *= -bounce_dampen;
            x_remainder = 0;
            x += -_dir; // ✅ nudge out of wall
            break;
        }
        x += _dir;
    }
}

// Vertical move
if (_move_y != 0) {
    var _dir = sign(_move_y);
    repeat(abs(_move_y)) {
        if (place_meeting(x, y + _dir, oObstacle)) {
            speed_v *= -bounce_dampen;
            y_remainder = 0;
            y += -_dir; // ✅ nudge out of floor/ceiling
            break;
        }
        y += _dir;
    }
}

// Flair
image_angle += speed_h;
